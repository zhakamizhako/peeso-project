...
class EasyServices extends Component {
    ...
    renderFreelancer(data, index) {
        return (<List.Item extra={"Book"} onPress={() => this.props.navigation.navigate("book")} multipleLine={false}>{data.firstname + " " + data.lastname}</List.Item>)
    }
    ...
    render() {
        return (<>
            <WingBlank>
                <ScrollView>
                    {this.state.data.map((entry, index) => { return (this.renderFreelancer(entry, index)) })}
                </ScrollView>
            </WingBlank>
        </>)
    }

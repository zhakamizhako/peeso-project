...
import { uploadFile, getFiles, cleanup } from '../../stores/modules/uploadmanager'
...
class MyUploads extends Component {
    ...
    renderMenu(entry, index, type) {
        return (<View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
            <Menu
                ...
                id={index}
                button={<View><Text style={{ fontSize: 20 }} onPress={(a) => {
    ...
    renderApplicant() {
        return (<View>
            <List>
                <List.Item extra={this.renderMenu(null, 1, 'sc')} >Skill Certificate
                    <View style={{ flexDirection: 'row', alignContent: 'space-between' }}>
                        <Text>{(!this.state.sc.error || !this.state.sc.file) && "None uploaded"}</Text>
                        <Text style={{ color: '#F00' }}>{this.state.sc.error}</Text>
                    </View>
                </List.Item>

                <List.Item extra={this.renderMenu(null, 2, 'ec')} >Educational Certificate
                    <View style={{ flexDirection: 'row', alignContent: 'space-between' }}>
                        <Text>{(!this.state.ec.error && !this.state.ec.file) && "None uploaded"}</Text>
                        <Text style={{ color: '#F00' }}>{this.state.ec.error}</Text>
                    </View></List.Item>
                <List.Item extra={this.renderMenu(null, 3, 'jv')} >
                    Job / Volunteering Certificate
                    <View style={{ flexDirection: 'row', alignContent: 'space-between' }}>
                        <Text>{(!this.state.jv.error && !this.state.jv.file) && "None uploaded"}</Text>
        ...
        return (<View>
            <List>
                ...
                {this.props.auth.loginData.company.is_overseas && (
                    <>
                        <List.Item>
                            No Objective Certificate (NOC)
                        </List.Item>
                        <List.Item>
                            Copy of Approved Job order issued by POEA
                        </List.Item>
                        <List.Item>
                            Affidavit of undertaking signed by the pres/VP of the company, LOA from POEA
                        </List.Item>
                        <List.Item>
                            Authority to Recruit issued by POEA
                        </List.Item>
                        <List.Item>

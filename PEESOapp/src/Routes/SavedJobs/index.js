...
class Trabaho extends Component {
    ...
    renderJobData(data, index, entrySaved) {
        return (<Card key={index} style={{ marginTop: 5 }}>
            ...
            )} extra={<TouchableOpacity onPress={() => this.removeJob(index)}><Icon style={{ alignSelf: "flex-end" }} size={30} color="black" name="book"></Icon></TouchableOpacity>} />
    ...
    render() {
        return (<>
            <WingBlank>
                {this.state.isLoading && (<View style={{ alignSelf: 'center', alignContent: 'center', marginVertical: '50%' }}><ActivityIndicator text="Loading Data..."> </ActivityIndicator></View>)}
                {this.state.data.length == 0 && !this.state.isLoading && (<View style={{ alignSelf: 'center', alignContent: 'center', marginVertical: '50%' }}><Text style={{ alignSelf: 'center', fontWeight: 'bold', fontSize: 25 }}>No Data :(</Text><Text style={{ alignSelf: 'center', fontSize: 18, marginTop: 15 }}>Why don't you try saving a few <Text style={{ color: 'blue' }} onPress={() => this.props.navigation.replace('trabaho')}>jobs</Text>?</Text></View>)}
                <ScrollView>
                    {console.log(this.state.data)}
                    {this.state.data.map((entry, index) => { return (this.renderJobData(entry.job, index, entry.created_at)) })}
                </ScrollView>
            </WingBlank>
        </>)
    }

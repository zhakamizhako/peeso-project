...
class EasyServices extends Component {
    ...
    renderJobData(data, index) {
        return (<List.Item extra={"PHP " + data.min + "-" + data.max} onPress={() => this.props.navigation.navigate("easyservicesfreelancers")}>{data.freelanceJob}</List.Item>)
    }
    ...
    render() {
        return (<>
            <WingBlank>
                <ScrollView>
                    {this.state.data.map((entry, index) => { return (this.renderJobData(entry, index)) })}
                </ScrollView>
            </WingBlank>
        </>)
    }

...
export const GET_EASY_SERVICES_SUCCESS =
  'easyservices/GET_EASY_SERVICES_SUCCESS';
...
export const GET_AVAILABLE_PERSONNEL_SUCCESS =
  'easyservices/GET_AVAILABLE_PERSONNEL_SUCCESS';
export const GET_AVAILABLE_PERSONNEL_ERROR =
  'easyservices/GET_AVAILABLE_PERSONNEL_ERROR';
export const GET_AVAILABLE_PERSONNEL_FAIL =
  'easyservices/GET_AVAILABLE_PERSONNEL_FAIL';
...
export const UPDATE_PROFILE_PIC_SUCCESS =
  'easyservices/UPDATE_PROFILE_PIC_SUCCESS';
...
export function book(data) {
  ...
  return (dispatch, getState) => {
    ...
    let hostname = API_HOST;
    axios
      .post(`${hostname}/v1/easyservice/book`, data, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${accessToken}`,
        },
...
actionHandlers[BOOK_SUCCESS] = (state, action) => {
  let newState;
  newState = objectAssign({}, state);
  newState.bookSuccess = true;
  newState.bookError = false;
  newState.bookData = action.payload.data;
  newState.accessToken = action.payload.accessToken;
  return newState;
};

actionHandlers[BOOK_FAIL] = (state, action) => {
  let newState;
  newState = objectAssign({}, state);
  newState.bookSuccess = false;
  newState.bookError = action.payload.error
    ? action.payload.error.message
    : action.payload.message;
  return newState;
};
